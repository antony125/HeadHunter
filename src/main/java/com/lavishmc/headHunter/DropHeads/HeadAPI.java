	...
	void loadTextures(String headsList, boolean logMissingEntities, boolean logUnknownEntities){
		...
		while(it.hasNext()){
			...
			while(textures.containsKey(value)){
				...
				if(++depth > 5){ pl.getLogger().severe("Likely infinite-loop: Redirects in head-textures.txt for "+e.getKey()); break; }
			}
			if(redirect != null){
				e.setValue(value);
...
//		SAVE_CUSTOM_LORE = pl.getConfig().getBoolean("save-custom-lore", false);
		...
		//---------- <Load Textures> ----------------------------------------------------------------------
		final String hardcodedList = FileIO.loadResource(pl.getClass(), "configs/head-textures.txt", /*defaultContent=*/"");
...
//		if(code == null || code.isEmpty()) return null;
		...
		if(nbSounds != null){
			...
			if(sound != null) try{SkullMeta.class.getMethod("setNoteBlockSound", NamespacedKey.class).invoke(meta, Sound.class.getMethod("getKey").invoke(sound));}
