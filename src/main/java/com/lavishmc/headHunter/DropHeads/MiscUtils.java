...
public final class MiscUtils{
	...
	public static final List<String> getLore(@Nonnull ItemStack item){
		if(useComponentsInsteadOfNBT){
			...
			final List<?> loreRaw = (List<?>)ReflectionUtils.get(loreField, item.getItemMeta());
			if(loreRaw == null) return null;
			return loreRaw.stream().map(CompConverter::jsonStrFromChatComp).toList();
		}
		else{
			RefNBTTagCompound tag = NBTTagUtils.getTag(item);
			RefNBTTagCompound display = tag.hasKey("display") ? (RefNBTTagCompound)tag.get("display") : new RefNBTTagCompound();
...
//						new TextHoverAction(HoverEvent.SHOW_ENTITY, convertEntityToJson(entity),
	...
	public static final ItemStack giveItemToEntity(Entity entity, ItemStack item){
		...
		if(entity instanceof LivingEntity){
			...
			if(equipment.getItemInMainHandDropChance() == 0F && equipment.getItemInMainHand().getType() == Material.AIR){ equipment.setItemInMainHand(item); return null; }
			if(equipment.getItemInOffHandDropChance() == 0F && equipment.getItemInOffHand().getType() == Material.AIR){ equipment.setItemInOffHand(item); return null; }
			return item;
		}
		return item;
	}
